...
for method, times in interfile_comparison_data.items():
    ...
    function = lambda _x,_a,_b,_c,_d: _a*_x**3+_b*_x**2+_c*_x+_d
    params = scipy.optimize.curve_fit(function, x, y, maxfev=5000)
    plt.rcParams.update({"text.usetex": True})
    func_str = f"${float_to_latex(params[0][0])} x^3 {'+' if params[0][1] > 0.0 else ''} {float_to_latex(params[0][1])} x^2  {'+' if params[0][2] > 0.0 else ''} {float_to_latex(params[0][2])} x {'+' if params[0][3] > 0.0 else ''} {float_to_latex(params[0][3])}$"
    axes.scatter(x, y)
