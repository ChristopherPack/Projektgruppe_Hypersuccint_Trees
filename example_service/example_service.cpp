...
std::ostream& operator<<(std::ostream& out, std::vector<bool> bitvetor) {
...
int main() {
    ...
    std::shared_ptr<pht::UnorderedTree<std::string>> xmlTree = pht::XMLReader::read("tree.xml");

    std::cout << "\n\nXML tree:\n" << xmlTree->toString();
    std::vector<std::shared_ptr<pht::UnorderedTree<std::string>>> componentSubtrees2 = pht::FarzanMunro<std::string>::decompose(xmlTree, 12);
    std::cout << "\n\nComponent trees:\n";
    ...
    pht::HypersuccinctTree::MiniTree miniTree = hst.getMiniTree(0);
    for(int index = 0 ; index < 8; index++) {
        std::cout << "MicroTreesinMiniTree" << index << ":  ";
        printBitvector(hst.getMiniTree(index).microTrees);
        std::cout << "MicroTreeFIDs:  ";
        printBitvector(hst.getMiniTree(index).FIDs);
        std::cout << "MicroTreeTypeVectors:  ";
        printBitvector(hst.getMiniTree(index).typeVectors);
        std::cout << "MicroTreeDummys:  ";
        printBitvector(hst.getMiniTree(index).dummys);
    }
